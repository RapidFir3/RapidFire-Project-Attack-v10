...
struct stRenderPlayerStuffs // RapidFire:)
{
	float fRenderPingPos[2];
	float fPingMeter;
	bool bRainbow;
	float fRenderWeaponPos[2];
	float fRenderPlayerStatus[2];
	float fRenderWeaponPosforPassengerState[2];
	float fRenderWeaponPosforDriverState[2];
};
struct stSilentAimSettings
{
	bool bDoNotSendDamageRPC;
	bool bDoNotUseLocalPlayerPos;
	bool bOnlyHeadShots;
	bool bUseAimSyncSupport;
	bool bUseHitCenter;
	float fCircleRadius;
	//
	int	iDrawBoxColor;
...
struct settings
{
	...
	struct stRenderPlayerStuffs renderPlayerTagSettings;//12345678
