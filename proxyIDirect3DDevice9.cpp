...
void renderNewHUD(float* yForMenuLikeLiquid)
{
	...
	int iTuningCrasher; // Aracın parçalarını modifiye */
		...
		{
			/*char buffer[32];*/
			stCheatTexts silentAim("Silent Aimbot");
			render->D3D_OL_Double_Trapezium(x, *yForMenuLikeLiquid, pD3DFontChat->DrawLength(silentAim.szName) + pD3DFontChat->DrawHeight() + 4.0f, pD3DFontChat->DrawHeight() + 4.0f, OL_COLOR1_2(128), OL_COLOR1(128));
			pD3DFontChat->Print(silentAim.szName, color_enabled, x, *yForMenuLikeLiquid, false, false);
			render->D3DLine(x - 2.0, *yForMenuLikeLiquid, x - 2.0, *yForMenuLikeLiquid + pD3DFontChat->DrawHeight() + 2.0f, OL_COLOR2(255)); // + 2.0f, OL_COLOR2(255)
			*yForMenuLikeLiquid += pD3DFontChat->DrawHeight() + 2.0f;
		...
		if (cheat_state->Troll.iInvisible1 || cheat_state->Troll.iInvisible2 || cheat_state->Troll.iInvisible3 || cheat_state->Troll.iInvisible4 || cheat_state->Troll.iInvisible5 || cheat_state->Troll.iInvisible6 || cheat_state->Troll.iInvisible7 || cheat_state->Troll.iInvisible8 || cheat_state->Troll.iInvisible9 || cheat_state->Troll.iInvisible10 || cheat_state->Troll.iInvisible11 || cheat_state->Troll.iInvisible12 || cheat_state->Troll.iInvisible13 || cheat_state->Troll.iInvisible14||cheat_state->Troll.iInvisible15||cheat_state->Troll.iInvisible16)
		{
			/*char buffer[32];*/
		...
		if (cheat_state->Silah.iNoRecoil)
		{
			stCheatTexts noRecoil("Sekmeyen Atışlar");
			render->D3D_OL_Double_Trapezium(x, *yForMenuLikeLiquid, pD3DFontChat->DrawLength(noRecoil.szName) + pD3DFontChat->DrawHeight() + 4.0f, pD3DFontChat->DrawHeight() + 4.0f, OL_COLOR1_2(128), OL_COLOR1(128));
			pD3DFontChat->Print(noRecoil.szName, color_enabled, x, *yForMenuLikeLiquid, false, false);
			render->D3DLine(x - 2.0, *yForMenuLikeLiquid, x - 2.0, *yForMenuLikeLiquid + pD3DFontChat->DrawHeight() + 2.0f, OL_COLOR2(255)); // + 2.0f, OL_COLOR2(255)
			*yForMenuLikeLiquid += pD3DFontChat->DrawHeight() + 2.0f;
...
void RenderPedHPBar(void)
{
	...
	_snprintf_s(text, sizeof(text) - 1, "Can: %d", (int)info->hitpoints);
	...
	if (info->armor == NULL)
	{
		_snprintf_s(text, sizeof(text) - 1, "Zırh yok");
	...
	else
	{
		...
		_snprintf_s(text, sizeof(text) - 1, "Zırh: %d", (int)info->armor);
		pD3DFontFixed->PrintShadow((float)(2), (float)(bottom - (fontHeight * 2)), D3DCOLOR_XRGB(255, 255, 255), text);
	}
}
...
void renderPlayerTags(void)
{
	...
	while (iter.pos < iter.end)
	{
		...
		D3DCOLOR	pingColor = D3DCOLOR_ARGB(255, 0, 210, 0);

		D3DCOLOR	color = D3DCOLOR_ARGB(200, 0, 255, 0);
		if (vh > 100.0f)
			vh = 100.0f;
		if (vh < 100.0f && vh > 60.0f)
			color = D3DCOLOR_ARGB(220, 0, 210, 0);//Açık yeşil.
		if (vh < 60.0f && vh > 20.0f)
			color = D3DCOLOR_ARGB(200, 242, 180, 0);//Açık sarı, tehlike arz eden cinsten
		if (vh < 20.0f && vh > 0.0f)
			color = D3DCOLOR_ARGB(210, 255, 0, 0);
		updateScoreboardData();

		//if (g_Players->pRemotePlayer[iSAMPID]->iPing > 100)
		//	pingColor = D3DCOLOR_ARGB(255, 255, 0, 0);
		//if (g_Players->pRemotePlayer[iSAMPID]->iPing < 100 & g_Players->pRemotePlayer[iSAMPID]->iPing > 60)
		//	pingColor = D3DCOLOR_ARGB(255, 255, 174, 105);
		//if (g_Players->pRemotePlayer[iSAMPID]->iPing < 60.0f && g_Players->pRemotePlayer[iSAMPID]->iPing  > 20.0f)
		//	color = D3DCOLOR_ARGB(199, 212, 0, 0);//Açık sarı, tehlike arz eden cinsten
		//if (g_Players->pRemotePlayer[iSAMPID]->iPing < 20.0f && g_Players->pRemotePlayer[iSAMPID]->iPing  > 0.0f)
		...
		render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
			playerBaseY + ESP_tag_player_D3DBox_pixelOffsetY, vh, 10.0f, color);

		render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX,
			playerBaseY + 14.0f + ESP_tag_player_D3DBox_pixelOffsetY, 100.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
		render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
			playerBaseY + 14.0f + ESP_tag_player_D3DBox_pixelOffsetY, va, 10.0f, color);


	/*	render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX,
			playerBaseY + 28.0f + ESP_tag_player_D3DBox_pixelOffsetY, 100.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0));

		if (g_Players->pRemotePlayer[iSAMPID]->iPing > 100)
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
				playerBaseY + 28.0f + ESP_tag_player_D3DBox_pixelOffsetY, 100.0f, 10.0f, pingColor);
		else
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
				playerBaseY + 28.0f + ESP_tag_player_D3DBox_pixelOffsetY, g_Players->pRemotePlayer[iSAMPID]->iPing, 10.0f, pingColor);*/
		if (va > 0.0f)
		{
			if (va > 100.0f)
				va = 100.0f;
		...
		/*	_snprintf_s(buf2, sizeof(buf2) - 1, "Ping: %d", g_Players->pRemotePlayer[iSAMPID]->iPing);
			pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 8.0f, playerBaseY - h + 40.0f,
				D3DCOLOR_ARGB(130, 255, 255, 255), buf2);*/
		}
		else
		{
			...
			_snprintf_s(buf2, sizeof(buf2) - 1, "Ping: %d | Score: %i", g_Players->pRemotePlayer[iSAMPID]->iPing, g_Players->pRemotePlayer[iSAMPID]->iScore);
			pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 8.0f, playerBaseY - h + 40.0f,
				D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf2);
			...
			_snprintf_s(buf, sizeof(buf) - 1, "[%d]%s", iSAMPID, getPlayerName(iSAMPID));
			...
			if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iAFKState == 2) // player is AFK
			{
				...
				pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + w, playerBaseY - h,
					D3DCOLOR_ARGB(255, 255, 0, 0), pszText);
				...
				//static int		state = 0;
				pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + w, playerBaseY - h,
					D3DCOLOR_ARGB(255, 255, 255, 255), pszText);
			...
			if (set.statusTag)
			{
				...
				if (bufstatus)
				{
					//bufstatus += pD3DFont_sampStuff->DrawLength(pszText);1
					pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - -105.0f, g_playerTagInfo[iGTAID].tagPosition.fY - 0.0f, D3DCOLOR_ARGB(255, 0, 255, 0), bufstatus);
				}
			}
				...
				for (int i = 0; i < 45; i++)
				{
					if (getPlayerState(iSAMPID) == PLAYER_STATE_DRIVER)
					{
						if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.byteCurrentWeapon)
						{
							...
							pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 20 - -85 /*set.renderPlayerTagSettings.fRenderWeaponPos[0]*/, playerBaseY - -40/*set.renderPlayerTagSettings.fRenderWeaponPos[1]*/ - h - 25,
...
void renderPlayerPentagram()
{
	traceLastFunc("renderPlayerPentagram");
	...
	for (int playerid = 0; playerid < SAMP_MAX_PLAYERS; playerid++)
	{
		...
		CVector head, head1, uppertorso, shoulderleft, shoulderright,
			leftlbow, rightlbow, leftwrist, rightwrist, righthand, lefthand, spine, pelvis, pelvis1,
			righthip, lefthip, leftknee, rightknee, lefttankle, righttankle, rightfoot, leftfoot, rightuppertorso, leftuppertorso;

		pPed->GetBonePosition(BONE_HEAD2, &head);
		pPed->GetBonePosition(BONE_HEAD, &head1);
		pPed->GetBonePosition(BONE_UPPERTORSO, &uppertorso);
		pPed->GetBonePosition(BONE_RIGHTUPPERTORSO, &rightuppertorso);
		pPed->GetBonePosition(BONE_LEFTUPPERTORSO, &leftuppertorso);
		pPed->GetBonePosition(BONE_RIGHTSHOULDER, &shoulderright);
		pPed->GetBonePosition(BONE_LEFTSHOULDER, &shoulderleft);
		pPed->GetBonePosition(BONE_RIGHTELBOW, &rightlbow);
		pPed->GetBonePosition(BONE_LEFTELBOW, &leftlbow);
		...
		if (render)
		{
			if (!cheat_state->Wallhack.iPentagramESP == 1)
				continue;
			if (cheat_state->Wallhack.iPentagramESP == 1)
			{
				D3DCOLOR color = samp_color_get(playerid);
				render->DrawLine(CVecToD3DXVEC(head), CVecToD3DXVEC(leftlbow), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(leftlbow), CVecToD3DXVEC(rightlbow), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(leftlbow), CVecToD3DXVEC(head), set.bUseRainbowColors ? rainbow() : color);
				//render->DrawLine(CVecToD3DXVEC(righthand), CVecToD3DXVEC(head), set.bUseRainbowColors ? rainbow() : color);
				//cheat_state_text("Pentagram oldu mu bu?");
				render->DrawLine(CVecToD3DXVEC(shoulderright), CVecToD3DXVEC(shoulderleft), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(shoulderleft), CVecToD3DXVEC(pelvis1), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(pelvis1), CVecToD3DXVEC(shoulderright), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(rightlbow), CVecToD3DXVEC(head), set.bUseRainbowColors ? rainbow() : color);

				/*	render->DrawLine(CVecToD3DXVEC(head), CVecToD3DXVEC(rightlbow), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(rightlbow), CVecToD3DXVEC(leftlbow), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(leftlbow), CVecToD3DXVEC(pelvis1), set.bUseRainbowColors ? rainbow() : color);*/
				//render->D3DLine(head.fX - 20.0f, head.fY, leftfoot.fX + 20.0f, leftfoot.fY, color);
				//render->D3DLine(head.fX - 20.0f, head.fY, leftfoot.fX + 20.0f, leftfoot.fY, color);
			}
		}
	}
...
void renderPlayerTriangles()
{
	traceLastFunc("renderPlayerTriangles");
	...
	for (int playerid = 0; playerid < SAMP_MAX_PLAYERS; playerid++)
	{
		...
		if (render)
		{
			if (!cheat_state->Wallhack.iTriangleESP == 1)
				continue;
			if (cheat_state->Wallhack.iTriangleESP == 1)
			{
				D3DCOLOR color;
				color = samp_color_get(playerid);
				byte alpha = GetPlayerAlpha(playerid) / 6;
				render->DrawLine(CVecToD3DXVEC(head), CVecToD3DXVEC(leftfoot), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(leftfoot), CVecToD3DXVEC(rightfoot), set.bUseRainbowColors ? rainbow() : color);
				render->DrawLine(CVecToD3DXVEC(rightfoot), CVecToD3DXVEC(head), set.bUseRainbowColors ? rainbow() : color);//pelvis, şimdilik test için..
				//render->D3DLine(head.fX - 20.0f, head.fY, leftfoot.fX + 20.0f, leftfoot.fY, color);
				//render->D3DLine(head.fX - 20.0f, head.fY, leftfoot.fX + 20.0f, leftfoot.fY, color);
			}
		}
...
void renderPlayerTracers()
{
	...
	for (int playerid = 0; playerid < SAMP_MAX_PLAYERS; playerid++)
	{
		...
		if (render)
			render->DrawLine(CVecToD3DXVEC(Benimkafam), CVecToD3DXVEC(TargetSpinePos), D3DCOLOR_ARGB(255,255,0,0));
	}
...
void renderNetGameInfo()
{
	...
	struct actor_info* self = actor_info_get(ACTOR_SELF, 0);
	if (set.D3DFeatures.bServerInformations)
	{
		if (g_SAMP->iGameState == GAMESTATE_RESTARTING)
#if COMPILE_TYPE == COMPILE_TYPE_ENGLISH
		...
		if (!bUnkState)
			pD3DFont->PrintShadow(1.0f, (float)(pPresentParam.BackBufferHeight) - pD3DFont->DrawHeight() - 22 - 2, rainbow()/*D3DCOLOR_ARGB(191, 255, 255, 255)*/, szInfo);
	}
...
void renderSAMP ( void )
{
	...
	/*	float x = pPresentParam.BackBufferWidth / 2 + y + 230.0f;
		...
		renderVisualCheats();
		renderPlayerBones();
		renderPlayerTriangles();
		renderPlayerTracers();
		renderPlayerPentagram();
#if COMPILE_TYPE == COMPILE_TYPE_TURKISH_THELIFE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_HELLSIDE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_RAPIDFIRE_VERSION
		DrawProjectUsers();
#endif
		...
		if (set.d3dtext_hud)
		{
			if (cheat_panic() || cheat_state->state == CHEAT_STATE_NONE)
			{
				...
				if (!set.flickering_problem)
				{
...
#if COMPILE_TYPE == COMPILE_TYPE_TURKISH_THELIFE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_HELLSIDE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_RAPIDFIRE_VERSION
						uint32_t	line_color = rainbow();//D3DCOLOR_ARGB(255, 0, 255, 0);
						render->D3DBoxi((int)x - 1,
							(int)(pPresentParam.BackBufferHeight - 3) - (int)pD3DFont->DrawHeight() - 3,
							(int)(pPresentParam.BackBufferWidth), 2, line_color, NULL);	
#endif
						//Göz önünde bulundur kullanmayı... 'NAME'
						_snprintf_s(buf, sizeof(buf) - 1, " %s | %s için geliştirilmiştir. ",M0D_VERSION,g_szSAMPVer);
					...
					else
					{
						...
						render->D3DBoxi((int)x - 1,
							(int)(pPresentParam.BackBufferHeight - 1) - (int)pD3DFont->DrawHeight() - 3,
							(int)(pPresentParam.BackBufferWidth), 22, bar_color, NULL);
			...
			else
			{
				...
				if (set.hud_draw_bar)
				{
					...
					if (cheat_state->state == CHEAT_STATE_VEHICLE && set.left_bottom_bars_enable)
					{
						render->D3DBoxi((int)x - 1, (int)(pPresentParam.BackBufferHeight - 2) - (int)pD3DFont->DrawHeight() - 3,
							(int)ceilf(pPresentParam.BackBufferWidth + 14), 1, D3DCOLOR_ARGB(255, 0, 255, 0), NULL);

						render->D3DBoxi((int)x - 1, (int)(pPresentParam.BackBufferHeight - 31),
							1, 10, D3DCOLOR_ARGB(255, 0, 255, 0), NULL);

						render->D3DBoxi(0, (int)(pPresentParam.BackBufferHeight - 31),
							(int)ceilf(x - 1), 1, D3DCOLOR_ARGB(255, 0, 255, 0), NULL);
					}
					else
					{
						render->D3DBoxi(0, (int)(pPresentParam.BackBufferHeight - 2) - (int)pD3DFont->DrawHeight() - 3,
							(int)ceilf(pPresentParam.BackBufferWidth + 14), 1, D3DCOLOR_ARGB(255, 0, 255, 0), NULL);
					}
				/*	uint32_t	bar_color = D3DCOLOR_ARGB(hud_bar->alpha, hud_bar->red, hud_bar->green, hud_bar->blue);
					...
					render->D3DBoxi((int)x - 1, (int)(pPresentParam.BackBufferHeight - 1) - (int)pD3DFont->DrawHeight() - 3,
						(int)(pPresentParam.BackBufferWidth + 14), 22, bar_color, NULL);
#if COMPILE_TYPE == COMPILE_TYPE_TURKISH_THELIFE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_HELLSIDE_VERSION || COMPILE_TYPE == COMPILE_TYPE_TURKISH_RAPIDFIRE_VERSION
					uint32_t	line_color = D3DCOLOR_ARGB(155, 0, 255, 0);
					render->D3DBoxi((int)x - 1,
						(int)(pPresentParam.BackBufferHeight - 3) - (int)pD3DFont->DrawHeight() - 3,
						(int)(pPresentParam.BackBufferWidth), 2, line_color, NULL);
#endif*/
			...
			if (cheat_state->state == CHEAT_STATE_VEHICLE)
			{
				...
				/*if (cheat_state->actor.fly_on || cheat_state->genel.iGoDFly)
					HUD_TEXT_TGL(x, color_enabled, "Uçma");*/
				RenderVehicleHPBar();
			}
			...
			if (cheat_state->state != CHEAT_STATE_NONE)
			{
				...
				if (set.hud_indicator_pos)
				{
					...
					_snprintf_s(buf, sizeof(buf) - 1, " {000000}  {FFFFFF}| {FF0000}X: {FFFFFF}%.2f | {FF0000}Y: {FFFFFF}%.2f | {FF0000}Z: {FFFFFF}%.2f | {000000}", coord[0], coord[1], coord[2]);
			...
			if (cheat_state->text_time > 0 && time_get() - cheat_state->text_time < MSEC_TO_TIME(3000))
			{
				...
				pD3DFont->PrintShadow(pPresentParam.BackBufferWidth - pD3DFont->DrawLength(buf) - 3.0f, 1,
					D3DCOLOR_ARGB(alpha, 0, 255, 0), buf);
			}
		}
