#include "main.h"

#define ID_SPINTYPE_OFF 0
#define ID_SPINTYPE_DOWN 1
#define ID_SPINTYPE_UP 2
#define ID_SPINTYPE_RANDOM 3

void SH_DoubleGun1(void);
void SH_DoubleGun2(void);


void SH_SAim_Draw(void);

void SH_AutoCbug1(void);
void SH_AutoCbug2(void);

void SH_NoRecoil(void);

void SH_NoReloadBug(void);
void SH_AutoSawn(void);

void SH_SawnDeagle(void);
void SH_Damage(void);
void SH_RapidFire(void); 
void SilahFonksiyonlari(void);
