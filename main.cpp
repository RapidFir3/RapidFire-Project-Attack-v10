...
void DataLogger(const char* fmt, ...)
{
	...
	{
		char	filename[512];
		snprintf(filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_attack_data_logger.log");
	...
	GetLocalTime(&time);
	fprintf(g_flLog, "[%02d:%02d:%02d] ", time.wHour, time.wMinute, time.wSecond);
	va_start(ap, fmt);
	vfprintf(g_flLog, fmt, ap);
	va_end(ap);
	fprintf(g_flLog, "\n");
