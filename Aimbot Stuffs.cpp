...
byte CalculateSpreadOffset(WORD targetID, float* target_pos, float* out_spread)
{
	...
	{
		CVector rbody;
		if (set.silentAim.bOnlyHeadShots)
		{
			byte bodyIDs[3] = { 8, 7, 6 };
			cheat_state->Silent.iSilentBone = bodyIDs[rand() % 2];
			Ped->GetTransformedBonePosition((eBone)cheat_state->Silent.iSilentBone, &rbody);
			out_spread[0] = rbody.fX - target_pos[0];
			out_spread[1] = rbody.fY - target_pos[1];
			out_spread[2] = rbody.fZ - target_pos[2];
			//cheat_state_text("Only Headshots: Aktif.");
		}
		else
		{
			...
			out_spread[1] = rbody.fY - target_pos[1];
			out_spread[2] = rbody.fZ - target_pos[2];
			//cheat_state_text("Only Headshots: Kapalý.");
	...
	else if (player_state == PLAYER_STATE_DRIVER || player_state == PLAYER_STATE_PASSENGER)
	{
		...
		Ped->GetTransformedBonePosition((eBone)cheat_state->Silent.iSilentBone, &head);
