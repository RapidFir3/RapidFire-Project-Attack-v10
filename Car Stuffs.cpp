	...
	if (cheat_state->Arac.iCarShooter)
	{
		...
		if (GetTickCount() - time >= 225)
		{
			for (int v = 0; v < SAMP_MAX_VEHICLES; v++)
			{
				...
				float fSpeed[3]; // float hýz, ondalýklý hýz dizi ile tanýmlý 0-1-2 dediðim gibi.. 3 dahil mi bilmiyorum. - RapidFire // 2021 - "Eylül'üm", 3 dahil deðil..
				if (KEY_DOWN(VK_LBUTTON))
				{
					vect3_copy(&oyuncubilgisi->base.matrix[4 * 3], fPos); // Veri kopyalamasý
					fPos[2] += 7.0f; // Z Pos + 4.0f;
...
void AF_CarShooter3()
{
	if (cheat_state->Arac.iCarShooter3)
	{
...
void AF_CarLagger(void)
{
	if (cheat_state->Arac.iCarLagger)
	{
		struct actor_info* ainfo = actor_info_get(ACTOR_SELF, NULL);
		if (!ainfo)
			return;
		...
		if (KEY_DOWN(VK_LBUTTON))
		{
			for (int car = NULL; car < SAMP_MAX_VEHICLES; car++)
			{
				if (GetTickCount() - time >= 200)
				{
					if (IsVehicleStreamed(car))
					{
						float fRoll[3] = { random_float(-100.0f, 100.0f) };
						int car_id = (int)(((DWORD)g_Vehicles->pGTA_Vehicle[car]) - (DWORD)pool_vehicle->start) / 2584;
						struct vehicle_info* vinfo = vehicle_info_get(car_id, 0);
						if (!vinfo)
							return;
						int vehicleID = getSAMPVehicleIDFromGTAVehicle(vinfo);
						g_RakClient->SendEnterPlayerVehicle(vehicleID, 0);
						float fDirection[3] = { vinfo->base.matrixStruct->up.X,vinfo->base.matrixStruct->up.Y,vinfo->base.matrixStruct->up.Z };
						//float fRoll[3] = {vinfo->base.matrixStruct->right.X,vinfo->base.matrixStruct->right.Y, vinfo->base.matrixStruct->right.Z};
						g_RakClient->SendDamageVehicle(vehicleID, 0, 4, 0, rand() % 2);
						g_RakClient->SendFullFakeUnoccupiedFuckingSyncData(vehicleID, 0, fDirection, &vinfo->base.matrix[12], vinfo->speed, vinfo->hitpoints, vinfo->spin, fRoll);
						g_RakClient->SendExitPlayerVehicle(vehicleID);
						cheat_state_text("lag123");
						time = GetTickCount();
					}
					time = GetTickCount();
				}
