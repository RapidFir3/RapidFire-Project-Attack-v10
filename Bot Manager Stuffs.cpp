...
void CBotEntity::SendFakeBulletData(uint16_t PlayerID, float Origin[3], float Target[3], float Center[3], uint8_t WeaponID, BYTE type)
{
	...
	if (PlayerID == g_Players->sLocalPlayerID)
		return;
	g_BotFuncs->BotSettings.sTargetID = PlayerID;
	//if()
...
void CBotEntity::ConnectAsZombie(uint16_t clientID)
{//N3X babam verdi lan! ~ RapidFire xd
	RakNet::BitStream bs;
	PlayerID playerId = g_BotFuncs->BotClient[clientID].pRakClient->GetPlayerID();
	...
	g_BotFuncs->BotClient[clientID].pRakClient->Send(&bs, PacketPriority::SYSTEM_PRIORITY, PacketReliability::RELIABLE, 0);
}
#endif