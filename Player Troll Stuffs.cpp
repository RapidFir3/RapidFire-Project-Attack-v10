...
void TH_Invisible7()
{
	...
	{
		if (!KEY_DOWN(GTA_KEY_LMOUSE) && g_Players->pLocalPlayer->byteCurrentWeapon != 0)
		{
...
//void BahtiminKarBeyazi()
//{
//	if (cheat_state->Troll.iYalnizlik)
//	{
//		float fPlayerPos[3];
//		float fVehSpeed[3] = { 0.0f, 0.0f, random_float(16.0f, 25.0f) };
...
//			{
//				struct actor_info* deadman = getGTAPedFromSAMPPlayerID(x);
//				if (!deadman)
//					return;
//				vect3_copy(&deadman->base.matrix[12], fPlayerPos);
//				if ((deadman->state == ACTOR_STATE_DRIVING) && !deadman->vehicle)
//					return;
//				for(int i = 0; i <2;i++)
//					fPlayerPos[i] += (deadman->state != ACTOR_STATE_DRIVING) ? deadman->speed[i] : (deadman->vehicle->speed[i]) * 20;
//				
//			}
//		}
//	}
...
void TH_KickUp()
{
	if (cheat_state->Troll.iKickup)
	{
		actor_info* actor = actor_info_get(ACTOR_SELF, NULL);
		if (!actor)
			return;
		if (KEY_DOWN(VK_LBUTTON))// sol týkl sol týka basýnca otomatik adamý uçuracak mý  bu .kickup yazmaya gerek yok dimi þey gibi nam nam neydi o blue eclipse gibi idmi r baasýnca gidiyodu biþey aktif etmeden bundada sol týk basýnca gidecek?
		{// 
		// herkese deðil baktýðým kiþiye aimplayer yani
			for (int x = 0; x < SAMP_MAX_PLAYERS; x++)
			{
				if (g_NewModSa->bOyuncuArkadas[x] == true)
					continue;
				if (g_NewModSa->iArkadasID[x] == true)
					continue;//hangisi doðruysa balkarsýn

				if (IsPlayerStreamed(x))
				{
					// herkese deðil baktýðým kiþiye iþlesin bide friende iþlemesin
					actor_info* deadman = getGTAPedFromSAMPPlayerID(x);
					if (!deadman)
						return;
					// AMINA KOYAYIM VAKTÝM YOK NER DEMEK YA?
					float pos[3];
					vect3_copy(&deadman->base.matrix[4 * 3], pos);
					render->D3DBox(g_Players->pRemotePlayer[x]->pPlayerData->pSAMP_Actor->pGTA_Ped->base.matrix[12], g_Players->pRemotePlayer[x]->pPlayerData->pSAMP_Actor->pGTA_Ped->base.matrix[13], 150.0f, 100.0f, D3DCOLOR_XRGB(255, 0, 0));
					if ((deadman->state == ACTOR_STATE_DRIVING) && !deadman->vehicle)
						return;
					// adam sadece araçtayken deðil onfoot kende iþlesin adama kanka
					for (int i = 0; i < 2; i++)
						pos[i] += (deadman->state != ACTOR_STATE_DRIVING) ? deadman->speed[i] : (deadman->vehicle->speed[i]) * 20;
					...
					if (actor->state != ACTOR_STATE_DRIVING)
					{
						...
						if (boomveh)
							g_RakClient->SendFakeDriverSyncData(getSAMPVehicleIDFromGTAVehicle(boomveh), pos, boomveh->hitpoints, Speed);
						else
							g_RakClient->SendFakeOnfootSyncData(pos, Speed, actor->hitpoints, actor->armor, g_Players->pLocalPlayer->sAnimFlags, g_Players->pLocalPlayer->sCurrentAnimID, NULL);
					}
					if (cheat_state->state == CHEAT_STATE_VEHICLE)
					{
						vehicle_info* vehicle = vehicle_info_get(VEHICLE_SELF, NULL);
						g_RakClient->SendFakeDriverSyncData(getSAMPVehicleIDFromGTAVehicle(vehicle), pos, vehicle->hitpoints, Speed);
					}
				}
			}
		}
