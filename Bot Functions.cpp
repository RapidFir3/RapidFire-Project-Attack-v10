...
void CBotFuncs::BotPlayerCrashers(uint16_t uiClientID)
{
	...
	*/
	...
	switch (set.Bots.BotCrasherType)
	{
		...
		{
			bool static foot_state = false;
			float fDetonatorPosition[3];
			float* coord = (cheat_state->state == CHEAT_STATE_VEHICLE) ? cheat_state->vehicle.coords : cheat_state->actor.coords;
			fDetonatorPosition[0] = coord[0], fDetonatorPosition[1] = coord[1], fDetonatorPosition[2] = coord[2];
			if (!foot_state)
			{
				...
				if (now_tick - tick > 150) // Delay 280'den 150'ye düþürüldü.
				{
					...
					onfootSync.fPosition[0] = fDetonatorPosition[0];
					onfootSync.fPosition[1] = fDetonatorPosition[1];
					onfootSync.fPosition[2] = fDetonatorPosition[2];
		...
		case BOT_CRASHER_TYPE_BULLETS:
		{
			...
			if (now_tick - tick > 250)
			{
				...
				if (cheack_valid_weaponid(WeaponID))
				{
					...
					g_BotFuncs->BotClient[BotSettings.CurrentClientID].pRakClient->Send(&bsUnoccupiedSync,HIGH_PRIORITY, UNRELIABLE_SEQUENCED, false); // Veriyi gönder
					tick = now_tick;
				}
			}
		}break;
		case BOT_CRASHER_TYPE_POSITION: // Passenger Crasher bs
		{
			if (now_tick - tick > 200)
			{
				...
				{
					g_BotClient->SendEnterPlayerVehicle(uiClientID, iVehicleID, 1);
		...
		{//Yukarýdaki kodlarýn hepsi 2019 yýlýna ait.. Ve bugün, 22 Eylül 2021... https://www.youtube.com/watch?v=iG-uPaB0_CM
			static DWORD dwSendRate = 200;
			if (GetTickCount() - dwSendRate > 200)
			{
				int PlayerID = GetClosestPlayerid(self, GetActorFlag(true, false, false, true, false, false, false, false));
				stOnFootData ofSync;
				ZeroMemory(&ofSync, sizeof(stOnFootData));
				vect3_copy(&self->base.matrix[12], ofSync.fPosition);
				g_BotEntity->WriteOnFootSync(&ofSync);
				g_BotClient->SendEnterPlayerVehicle(uiClientID, iVehicleID, 0);
				stUnoccupiedData unocData;
				ZeroMemory(&unocData, sizeof(stUnoccupiedData));
				unocData.sVehicleID = iVehicleID;
				vect3_copy(&vehicle->base.matrix[12], unocData.fPosition);
				unocData.fMoveSpeed[0] = vehicle->speed[0];
				unocData.fMoveSpeed[1] = vehicle->speed[1];
				unocData.fMoveSpeed[2] = vehicle->speed[2];
				unocData.fRoll[0] = 1.0f;
				unocData.fRoll[1] = 1.0f;
				unocData.fRoll[2] = 1.0f;
				unocData.fDirection[0] = vehicle->base.matrix[4 * 1 + 0];
				unocData.fDirection[1] = vehicle->base.matrix[4 * 1 + 1];
				unocData.fDirection[2] = vehicle->base.matrix[4 * 1 + 2];
				unocData.fTurnSpeed[0] = -5000.0f;//negative speedin mevzusu zaten burada..
				unocData.fTurnSpeed[1] = -5000.0f;
				unocData.fTurnSpeed[2] = -5000.0f;
				unocData.fHealth = vehicle->hitpoints;
				unocData.byteSeatID = 0;
				g_BotEntity->WriteUnoccupiedSync(&unocData);
				g_BotClient->SendExitPlayerVehicle(uiClientID, iVehicleID);
				dwSendRate = GetTickCount();
			}
		}break;
