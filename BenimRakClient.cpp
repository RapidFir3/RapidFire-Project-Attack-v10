...
void MyRakClient::TakeDamage(int iPlayerID, float fDamage, int iWeaponID, int iBodyPart)
{
	RakNet::BitStream bsGiveDamage;//al götüne sok qwe | true=alýnan hasar | false=verilen qwe napim qwe
	bsGiveDamage.Write<bool>(true);
	bsGiveDamage.Write<WORD>(iPlayerID);
	bsGiveDamage.Write<float>(fDamage);
	bsGiveDamage.Write<int>(iWeaponID);
	bsGiveDamage.Write<int>(iBodyPart);
...
void MyRakClient::SendFakePassengerSyncData(WORD vehicleid, float fPos[3], float HealthCar, uint8_t WeaponID, int SeatID)
{
...
void MyRakClient::SendFakeDriverSyncData(int iSAMPVehicleID, float fVehiclePos[3], float fVehicleHealth, float fMoveSpeed[3])
{
	...
	struct actor_info *pSelfActor = actor_info_get(ACTOR_SELF, ACTOR_ALIVE);
	...
	InCarData.sTrailerID = iSAMPVehicleID;
...
void MyRakClient::SendDamage(uint16_t uiPlayerID, float fDamageAmountToSend, int iWeaponID, int iBodyPart)
{
	...
	bsSendDamage.Write<bool>(false);
	bsSendDamage.Write<WORD>(uiPlayerID);
	bsSendDamage.Write<float>(fDamageAmountToSend);
	bsSendDamage.Write<int>(iWeaponID);
	bsSendDamage.Write<int>(iBodyPart);
	g_RakClient->RPC(RPC_GiveTakeDamage, &bsSendDamage, HIGH_PRIORITY, UNRELIABLE_SEQUENCED, 0, FALSE);
	cheat_state_text("Damage must be sent.");
}
