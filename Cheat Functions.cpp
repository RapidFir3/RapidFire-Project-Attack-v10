...
int GetClosestPlayerid(actor_info* player, int flugs)
{
	...
	for (int id = NULL; id < SAMP_MAX_PLAYERS; id++)
	{
		...
		if ((flugs & ACTOR_NOT_ME) && actor == player)
			continue;
		if ((flugs & ACTOR_ALIVE) && actor->hitpoints <= 1.0f)
			continue;
...
int GetAimingPlayer(bool bFromWhenPress)
{
		...
		|| g_NewModSa->iAimPlayerID == -1)
	{
		for (int i = 0; i < SAMP_MAX_PLAYERS; i++)
		{
			...
			{
				fNearestDistanceScreen = fCurrentDistanceScreen;
				if (cheat_state->Silent.bSilent || cheat_state->Silah.iRapidFire ||cheat_state->Silah.bWeaponDamageMultiplier || cheat_state->Silah.bSilentAimV2)
				{
	...
	if (cheat_state->Silent.bSilent || cheat_state->Silah.iRapidFire || cheat_state->Silah.bWeaponDamageMultiplier)
	{
